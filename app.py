...
def init_db():
    ...
    # ===== MIGRAÇÃO DE PREÇOS (corrige “itens perderam valores”) =====
    # Alguns bancos antigos tinham coluna "price" (texto) e/ou "price_cents" vazio.
    has_price_text = table_has_column(db, "products", "price")

    if has_price_text:
        # Se price_cents não existir (muito raro com seu CREATE atual), cria
        if not table_has_column(db, "products", "price_cents"):
            db.execute("ALTER TABLE products ADD COLUMN price_cents INTEGER NOT NULL DEFAULT 0;")
            db.commit()

        # Migra linha a linha quando price_cents estiver 0 e houver price preenchido
        rows = db.execute("SELECT id, price_cents, price FROM products;").fetchall()
        for r in rows:
            try:
                pid = int(r["id"])
                pc = int(r["price_cents"]) if r["price_cents"] is not None else 0
                pt = (r["price"] or "").strip()
            except Exception:
                continue
            ...
            # pt pode vir "R$ 10,00" ou "10,00" ou "10.00" etc
            try:
                cents = parse_price_to_cents(pt)
                if cents > 0:
    ...
    pcur = db.execute("SELECT COUNT(*) as c FROM products;")
    if pcur.fetchone()["c"] == 0:
        ...
        db.executemany(
            """
            INSERT INTO products (name, description, price_cents, image_url, category_id, category, is_active, is_promo, promo_price_cents)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
            """,
            seed,
        )
...
def parse_price_to_cents(raw: str) -> int:
    ...
    if "," in s:
        s = s.replace(".", "").replace(",", ".")
    # se não tiver vírgula, pode ser ponto decimal (15.5) ou inteiro
    value = float(s)
...
def admin_add():
    ...
    db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
        """,
        (name, description, price_cents, image_url, category_id, None, is_active, is_promo, promo_price_cents),
    )
    db.commit()
...
def admin_edit_post(pid):
    ...
    db.execute(
        """
        UPDATE products
        SET name=?, description=?, price_cents=?, image_url=?, category_id=?, is_active=?, is_promo=?, promo_price_cents=?
        WHERE id=?;
        """,
